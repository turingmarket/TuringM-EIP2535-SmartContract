...
contract VaultFacet is IVaultFacet, VaultBase, AccessControlBase, Facet {
    function VaultFacet_init(
        ...
        _setFunctionAccess(
            this.adminWithdrawPaymentToken.selector,
            roleA,
            true
        );
    ...
    function adminWithdrawPaymentToken(
    ...
    function adminUpdateVaultOperator(
        uint256 vaultId,
        address operator
    ) external whenNotPaused protected {
        require(vaultId < s.vaults.length, "Invalid vault id");
        s.vaults[vaultId].operator = operator;
        emit VaultOperatorUpdated(vaultId, operator);
    }

    function createVault(
        Vault memory vault_
    ) external whenNotPaused protected returns (uint256 vaultId) {
        ...
        if (vault_.vaultType == VaultType.Vc) {
            ...
            require(
                vault_.paymentTokenAddress != address(0),
    ...
    function depositToken(
        ...
        require(
            IERC20(vault.tokenAddress).transferFrom(
                msg.sender,
                address(this),
                amount
            ),
    ...
    function payoutToken(
        ...
        require(
            IERC20(vault.tokenAddress).transfer(to, amount),
            "Transfer failed"
        );
    ...
    function investToken(
        ...
        require(
            IERC20(vault.paymentTokenAddress).transferFrom(
                userAddress,
                address(this),
                paymentAmount
            ),
            "Transfer payment token failed"
        );
        ...
        if (canRefund == false) {
            ...
            s.withdrawablePaymentTokenMap[
                vault.paymentTokenAddress
            ] += paymentAmount;
        }
    ...
    function claimUnlockedTokens(
        ...
        uint256 availableAmount = schedule.allocationAmount -
            schedule.claimedAmount;
        require(
            availableAmount > 0 && availableAmount >= amount,
            "Insufficient availableAmount"
        );
        ...
        vault.balance -= amount;
        require(
            IERC20(vault.tokenAddress).transfer(userAddress, amount),
            "Transfer failed"
        );
    ...
    function quitInvestRefund(
        ...
        Vault storage vault = s.vaults[schedule.vaultId];
        s.withdrawablePaymentTokenMap[vault.paymentTokenAddress] += schedule
            .paymentAmount;
    }

    // TODO: add more level of security check
    function doInvestRefund(
        uint256 scheduleId,
        uint256 nonce,
        bytes memory userSig
    ) external protected whenNotPaused nonReentrant {
        ...
        require(
            schedule.startTime + schedule.canRefundDuration < block.timestamp,
            "Refund waiting time span not reach yet(should over 1 year)"
        );

        address userAddress = schedule.userAddress;

        bytes memory encodedDataUser = abi.encode(
            TYPEHASH_INVEST_DO_REFUND,
            scheduleId,
            nonce
        );
        _verifySignature(userAddress, userSig, encodedDataUser);
        ...
        vault.paymentAmount -= schedule.paymentAmount;
        require(
            IERC20(vault.paymentTokenAddress).transfer(
                userAddress,
