...
contract VoteFacet is IVoteFacet, VoteBase, AccessControlBase, UserNonceBase, Facet {
    function VoteFacet_init() external onlyInitializing {
        // _setFunctionAccess(this.setVoteToken.selector, ROLE_ADMIN, true);
    ...
    function createProposal(bytes32 descHash, uint256 duration) external protected {
        ...
        s.proposals.push(Proposal({descHash: descHash, startTime: block.timestamp, duration: duration, yesVotes: 0, noVotes: 0}));
    ...
    function vote(VoteParams memory params, bytes memory userSig) external protected {
        ...
        require(s.userVoteProposalMap[userAddress][proposalId] == false, "User has already voted on this proposal");
    ...
    function syncVotingPowerFromStakeScheduleIds(address userAddress, uint256 stakeScheduleId) external protected {
        ...
        for (uint256 i = 0; i < arr.length; i++) {
            if (stakeScheduleId == arr[i]) {
                StakeSchedule memory schedule = s.stakeSchedules[stakeScheduleId];
