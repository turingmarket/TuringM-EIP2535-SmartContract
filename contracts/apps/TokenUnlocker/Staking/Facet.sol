...
contract StakingFacet is IStakingFacet, StakingBase, VoteBase, AccessControlBase, UserNonceBase, Facet {
    function StakingFacet_init() external onlyInitializing {
        // _setFunctionAccess(this.setStakingToken.selector, ROLE_ADMIN, true);
        // _setFunctionAccess(this.stake.selector, ROLE_ADMIN, true);
    ...
    function stake(
        ...
        s.stakeSchedules.push(
            StakeSchedule({userAddress: userAddress, tokenAddress: tokenAddress, amount: amount, startTime: block.timestamp, isUnstaked: false})
        );
    ...
    function unstake(uint256 scheduleIndex, uint256 nonce, bytes memory userSig) external whenNotPaused protected nonReentrant {
        require(scheduleIndex < s.stakeSchedules.length, "Invalid schedule index");
