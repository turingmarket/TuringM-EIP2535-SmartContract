...
import {IDiamondCutBase} from "./IBase.sol";
import {DiamondCutStorage} from "./Storage.sol";
import {MULTI_INIT_ADDRESS} from "../../Constants.sol";
...
abstract contract DiamondCutBase is IDiamondCutBase {
    ...
    function _addFacet(address facet, bytes4[] memory selectors) internal {
        DiamondCutStorage.Storage storage $ = DiamondCutStorage.load();
        ...
        for (uint256 i = 0; i < selectors.length; i++) {
            ...
            // slither-disable-next-line unused-return
            $.facetSelectors[facet].add(selector);
        }
    }

    function _replaceFacet(address facet, bytes4[] memory selectors) internal {
        DiamondCutStorage.Storage storage ds = DiamondCutStorage.load();
