...
contract VaultFacet is IVaultFacet, VaultBase, AccessControlBase, Facet {
    ...
    function doInvestRefund(uint256 scheduleId, uint256 nonce, bytes memory userSig) external protected whenNotPaused nonReentrant {
        ...
        require(schedule.startTime + schedule.canRefundDuration < block.timestamp, "Refund waiting time span not reach yet(should over 1 year)");
    ...
    function getUnlockedSchedulesList(
        address user,
        uint256 page,
        uint256 pageSize
    ...
    function getVaultsLength() external view returns (uint256 vaultId) {
